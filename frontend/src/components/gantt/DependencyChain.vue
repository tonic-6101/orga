...
import { Link, Plus, CircleDot, Lock, Circle, Check, ChevronDown, X } from 'lucide-vue-next'
  ...
  <div class="dependency-chain">
    ...
    <div class="flex items-center justify-between mb-3">
      ...
      <button
        ...
        class="text-xs text-accent-500 hover:text-accent-600 flex items-center gap-1"
      >
    ...
    <div v-if="isAddingDep" class="mb-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 space-y-2">
      ...
      <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
        <CircleDot class="w-3 h-3 text-accent-500 shrink-0" aria-hidden="true" />
      ...
      <input
        ...
        class="w-full px-2.5 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 focus:outline-none focus:border-accent-500 focus:ring-1 focus:ring-accent-500"
      />
      ...
      <div class="max-h-36 overflow-y-auto rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
        <div
          ...
          class="flex items-center gap-2 px-2.5 py-2 hover:bg-accent-50 dark:hover:bg-gray-700 cursor-pointer transition-colors border-b border-gray-100 dark:border-gray-700 last:border-b-0"
        >
          <Plus class="w-3 h-3 text-gray-300 dark:text-gray-600 shrink-0" aria-hidden="true" />
    ...
    <div class="bg-gray-50 rounded-lg p-3">
      ...
      <div v-if="!hasDependencies && !hasDependents" class="text-center py-4 text-sm text-gray-400">
        <Link class="w-6 h-6 mb-2 opacity-50" aria-hidden="true" />
        <p>{{ __('No dependencies defined') }}</p>
        <button
          @click="isAddingDep = true; newDepSearch = ''; selectedDepTask = ''"
          class="mt-2 text-accent-500 hover:text-accent-600"
        >
      ...
      <div v-else class="space-y-1">
        ...
        <div v-if="hasDependencies" class="space-y-1">
          ...
          >
            ...
            >
              ...
              >
                <Check v-if="isCompleted(dep.status)" class="w-3 h-3" aria-hidden="true" />
                <Circle v-else class="w-1.5 h-1.5" aria-hidden="true" />
              </span>
        ...
        <div class="px-2 py-2 bg-accent-50 border border-accent-200 rounded-lg">
          <div class="flex items-center gap-2">
            <span class="w-5 h-5 rounded-full bg-accent-500 flex items-center justify-center text-white text-xs shrink-0">
              <Circle class="w-1.5 h-1.5" aria-hidden="true" />
            </span>
            <span class="text-sm font-medium text-accent-700 flex-1 truncate">{{ taskName }}</span>
            <span class="text-[10px] text-accent-500 bg-accent-100 px-1.5 py-0.5 rounded">
              {{ __('Current') }}
            </span>
          </div>
        </div>
    ...
    <div class="mt-3 text-[10px] text-gray-400 flex items-center gap-3">
      <span class="flex items-center gap-1">
        <Check class="w-3 h-3 text-green-500" aria-hidden="true" /> {{ __('Completed') }}
      </span>
      <span class="flex items-center gap-1">
        <Circle class="w-1 h-1 text-gray-400" aria-hidden="true" /> {{ __('Pending') }}
      </span>
    </div>
  </div>
</template>
